...
def makeFigure9():
    ...
    IT = np.loadtxt('output_objectives.csv',delimiter=',',skiprows=1)
    DPS = np.loadtxt('dps_output_objectives.csv',delimiter=',',skiprows=1)
    ...
    #read in robustness
    ITrobustness = np.loadtxt('robustness_it.txt',delimiter=' ')
    ITrobustness = ITrobustness*100
    DPSrobustness = np.loadtxt('robustness_dps.txt',delimiter=' ')
    ...
    IT_pts, DPS_pts, ideal_pt = makeSubPlot(IT, ITrobustness, DPS, \
            DPSrobustness, objs_min, objs_max, ax, fig)
